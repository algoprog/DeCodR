...
from utils import EmbeddingMemoryQueue
...
def run(cfg: TrainingConfig):
    ...
    memory_queue = EmbeddingMemoryQueue(768)
    ...
    for epoch in range(cfg.train_args.max_epochs):
        ...
        for queries, passages, qid, pid in dataloader_train:
            ...
            loss = loss_fn(q_embs, qid_tensor, ref_emb=p_embs, ref_labels=qid_tensor)

            if len(memory_queue) > 0:
                memory_embs, memory_labels = memory_queue.get_embeddings()
                loss += loss_fn(q_embs, qid_tensor, ref_emb=memory_embs, ref_labels=memory_labels)
            ...
            if step % gradient_accumulation_steps == 0 and (
                ...
                if cfg.train_args.log:
                    wandb.log({'loss': running_loss, 'step': step})
            ...
            if (
            ...
            ) % cfg.train_args.steps_between_val == 0 and (
                step > gradient_accumulation_steps
            ):
                ...
                if cfg.train_args.log:
                    wandb.log({'ndcg@10': ndcg, 'step': step, 'mrr': mrr})

            if (
                (step // gradient_accumulation_steps)
                % cfg.train_args.steps_between_save
                == 0
